...
//raspicam lib from https://github.com/cedricve/raspicam
//to use this, add lib : raspicam, raspicam_cv, mmal, mmal_core, mmal_util
...
std::vector<int> cam_streaming_converting_param; //set jpg quality in percent
...
void web_server_handle_request(void* ptr_data) {
    while (server_system_signal_received == 0) {
        //poll the server
        mg_mgr_poll(&mgr, 50);
    }
...
int main(int argc, char** argv) {
    ...
    //setup ipcam
    //    VideoCapture ipcam("http://192.168.1.252:8090/?action=stream");
    //    //if(ipcam.open("http://192.168.1.252:8090/?action=stream")){
    //    if(ipcam.isOpened()){
    //        printf("%dx%d\n", ipcam.get(CV_CAP_PROP_FRAME_WIDTH), ipcam.get(CV_CAP_PROP_FRAME_HEIGHT));
    //    }
    //    else{
    //        printf("open fail\n");
    //    }
...
//    cam_streaming_converting_param = {CV_IMWRITE_JPEG_QUALITY, 50};
    cam_streaming_converting_param.push_back(CV_IMWRITE_JPEG_QUALITY);
    cam_streaming_converting_param.push_back(50);
    ...
    thpool_add_work(thpool, web_server_handle_request, NULL);
